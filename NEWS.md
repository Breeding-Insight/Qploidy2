...
* Bugfix in `re_standardize`: markers in windows called as CN=0 (where `CN_call` is `NA`) were not being masked before re-standardization because `NA != ploidy.standardization` evaluates to `NA` rather than `TRUE`. Both code paths (`use_estimated_dosages = TRUE/FALSE`) now explicitly include `is.na(CN_call)` in the masking condition, so CN=0 markers are correctly excluded from standardization clusters.
