...
count_breaks_plot <- function(counts,
  ...
  paired_palette <- c("#A6CEE3", "#1F78B4", "#B2DF8A", "#33A02C", "#FB9A99", "#E31A1C", "#FDBF6F", "#FF7F00", "#CAB2D6", "#6A3D9A", "#FFFF99", "#B15928")
  if(any(colnames(counts) == "ploidy")) ploidycolors <- set1_palette[1:length(unique(counts$ploidy))]
  mycolors <- colorRampPalette(paired_palette)(length(unique(counts$LG)))
  set.seed(20)
