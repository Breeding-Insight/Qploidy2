...
if (getRversion() >= "2.15.1") utils::globalVariables(
  c("starts_all", "starts", "outlier", "WindowID", "z_mean", "Chr", "region_id", "w_baf", "SampleName", "Position", "baf", "Start", "End", "prob_call", "CN_call", "CN_reliability",
...
plot_cn_track <- function(hmm_CN,
  ...
  # -------- bottom panel: CN segments (color = CN reliability) --------
  if (!("prob_call" %in% names(x))) stop("prob_call column missing in CN plot data.")
  x$prob_call <- as.numeric(x$prob_call)
  # CN_reliability combines HMM confidence and BAF support; fall back to prob_call for old objects
  if (!"CN_reliability" %in% names(x)) {
    x$CN_reliability <- x$prob_call
    cn_rel_label <- "P(CN call)"
  } else {
    cn_rel_label <- "CN reliability"
  }
  ...
  p_cn <- ggplot(x) +
    ...
    geom_segment(
      data = x_seg,
      aes(x = Start, xend = End, y = CN_call, yend = CN_call, color = CN_reliability),
      linewidth = 2, lineend = "butt"
    ) +
    # Points for single-marker windows
    geom_point(
      data = x_pt,
      aes(x = Start, y = CN_call, color = CN_reliability),
      size = 2
    )
  if (show_window_lines) {
  ...
  p_cn <- p_cn +
    ...
    scale_color_viridis_c(name = cn_rel_label, limits = c(0, 1), option = "D", direction = -1) +
