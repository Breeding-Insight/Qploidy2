...
Among the first 50 samples, sample `85-117` differ between previous standardization and re-standardization results.

```{r}
# Same plots can be generated
plot_karyotype(re_multi_esti, sample_name = "85-2", nrow = 2, color_by = "post_max_CN")
```

In this case, there are a small fragment with lower copy number (CN 3) on part of chromosome 7. And a higher CN in chromosome 8. 
If you prefer to remove the dashed line showing the missing parts. Just set the
`add_ghost` argument to FALSE:

```{r}
# Same plots can be generated
plot_karyotype(re_multi_esti, sample_name = "85-2", nrow = 2, color_by = "post_max_CN", add_ghost = FALSE)
```
...
plot_karyotype(multi_esti, sample_name = "85-2", nrow = 2, color_by = "post_max_CN")
