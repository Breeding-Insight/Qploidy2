...
* **Supported Sequencing Platforms**: Your marker data originates from Axiom or Illumina genotyping arrays, targeted sequencing platforms 
(e.g., DArTag, GTseq, AgriSeq), or whole-genome sequencing (WGS) platforms. 
* **Batch effect**: All DNA samples were prepared following the same library preparation protocol 
*(check [this section](#check-for-batch-effect-before-standardization) for more details)*
...
* **Minimum previous knowledge**: You know the ploidy of at least a subset (rough minimum of 60 samples) or you know the most common ploidy in the dataset.
The higher the number of samples with known or common ploidy, the better the results
* **Heterozygosity**: Your dataset includes heterozygous samples

The Qploidy2 B-allele frequency (BAF) model selection and HMM methodologies **are effective** under the following conditions:

* **Standardized data**: The ratio or BAF distributions of your samples present clear peaks corresponding to different copy numbers
* **Total read depth**: The total read depth or z-scores variations within a sample follows a normal distribution, with the mean shifting according to the copy number
* **Heterozygosity**: The sample includes heterozygous loci, which are essential for BAF-based ploidy estimation.
...
* **Sequencing depth**: If allele counts are based on a sequencing platform, the sequencing should have enough depth to capture dosages of the 
highest possible copy number in the dataset
* **Number of markers**: The number of markers is sufficient to provide a good resolution for the BAF distributions
* **CN call resolution**: The larger the amount of markers, the smaller the copy number variations that can be detected
...
You can access a description of all parameters in `?standardize`. Here we will focus only a few of them:
...
Other than the minimum number of clusters, the standardization step also applies some other filters:

- `threshold.missing.geno`: Maximum fraction of missing datapoints allowed per marker. Markers with a higher fraction will be removed.
- `threshold.missing.sample`: Maximum fraction of missing datapoints allowed per sample. Samples with a higher fraction will be removed.
- `min.depth`: Minimum read depth (R) threshold. Datapoints with R below this value will have their allele ratio set to missing and be excluded from BAF and Z-score computations.
- `max.depth`: Maximum read depth (R) threshold. Datapoints with R above this value will have their allele ratio set to missing and be excluded from BAF and Z-score computations.

Here is a summarized table with the arguments presented:

| Argument | Description |
|----------|-------------|
| `genos` | Data.frame with samples and their dosage based on known or most common ploidy for standardization |
| `ploidy.standardization` | Predominant ploidy for the dataset used to call the dosages in the object `genos` |
| `threshold.n.clusters` | Minimum number of genotype dosage clusters required for a marker to be retained |
| `threshold.missing.geno` | Maximum fraction of missing datapoints allowed per marker |
| `threshold.missing.sample` | Maximum fraction of missing datapoints allowed per sample |
| `min.depth` | Minimum read depth (R) threshold |
| `max.depth` | Maximum read depth (R) threshold |

Explore more options with `?standardize`.

If your data comes from sequencing (and not hybridization), filtering by depth is important for quality control.
To decide on the minimum and maximum of depth, you may want to take a quick look on the total depth distribution:
...
- Grid approach BAF model selection: does not use an EM algorithm but a grid of parameters to be tested, 
which is less time- and computationally demanding but may not provide the best parameters for this step.
The results from this step are initial parameters for the HMM, 
which will be further optimized by the EM algorithm implemented in the HMM.


This current tutorial apply the "grid approach". Check tutorial [Qploidy2 and nQuack integration]() for running the EM approach using nQuack.
...
selected_model_raw <- select_best_baf_model(baf_vec = sample_ratios,
                                            cn_grid= c(2,3,4,5,6),
                                            bw = c(0.02, 0.04, 0.06, 0.08, 0.1), # Default values
                                            plot = TRUE)
...
**For this dataset, standardization is required for better results**. We will move forward just with the standardized object.
...
```{r}
plot_karyotype(res, sample_name = "J432", nrow = 2)
```

You can add color by the CN call final likelihood:

```{r}
plot_karyotype(res, sample_name = "J432", nrow = 2, color_by = "post_max_CN")
```
...
- **Interpreting the plot**: bright segments indicate high-confidence calls backed by both
  ...
  later in this tutorial to mask low-confidence windows before export.
...
# Filter by CN call according to quality metrics

`filter_hmm_CN()` masks CN calls that do not meet user-defined quality thresholds by replacing `CN_call`,
`post_max`, `CN_reliability`, and all `post_CN*` posteriors with `NA`.  Masked windows are treated as
missing by all downstream plotting and export functions.
...
* `min_CN_reliability` — joint confidence combining HMM posterior probability and BAF weight.
  ...
  Windows supported only by z-score (e.g., all-homozygous chromosomes) cannot exceed
  `z_no_baf_scale × post_max` in reliability (default cap = `0.25 × post_max`).
* `min_post_max` — the raw HMM posterior probability for the called CN state, regardless of BAF support.
  Useful when you want to retain z-score-only calls but still discard low-confidence HMM calls.
...
After curating individual sample results you can get a high-level overview of the entire population using two complementary tools.

`count_types()` classifies every sample as **euploid** (all chromosomes match the sample-level copy number),
**aneuploid** (one or more chromosomes carry a different but uniform CN — whole-chromosome gain or loss), or
**segmental** (at least one chromosome has windows with mixed CN, indicating a partial chromosomal change).
It prints a summary table to the console and returns a named list of sample-ID vectors that you can use for downstream filtering:
...
Use the provided list to visualize specific sample type:

```{r}
plot_karyotype(multi_esti, 
               sample_name = types_df$aneuploid[14], color_by = "CN_reliability")

# Option to remove the ghost missing chromosome:
plot_karyotype(multi_esti, 
               sample_name = types_df$aneuploid[14], color_by = "CN_reliability", add_ghost = FALSE)

plot_karyotype(multi_esti_filtered, 
               sample_name = types_df_filtered$segmental[1], color_by = "CN_reliability")
```

`compare_cn_track_summary()` produces a karyotype-style matrix with samples in rows and chromosomes in columns.
Samples are ordered by ascending sample-level CN and, within each CN block, by ploidy category (euploid → aneuploid → segmental).
Chromosomes with mixed CN within their windows are shown in purple (segmental aneuploidy); others follow the same blue/gray/red palette as `compare_cn_track()`.
Strong horizontal lines separate CN blocks; dashed lines separate the ploidy-category sub-blocks; group labels appear to the right:
...
For this dataset, there are a few changes for some samples. 

# Refine results for a single sample

If this instability still leaves doubts about 
...
```{r}
plot_karyotype(multi_esti_filtered, sample_name = "85-177", nrow = 2, color_by = "post_max_CN")

p <- plot_cn_track(hmm_CN = multi_esti_filtered,
                   qploidy_standarize_result = re_qploidy_standardization,
                   sample_id = "85-177",
...
plot_karyotype(res_tweak, sample_name = "85-177", nrow = 2, color_by = "post_max_CN")
...
**WARNING**: Qploidy2 HMM results accuracy depends on many factors that sometimes
the call likelihoods are not capable to detect. We strongly suggest to review the 
diagnostic plots for non-expected results. The `GenoBrew` tool described below
is a great resource to make the graphics evaluation and results curation process quick.
...
class(re_multi_esti_filtered) # result from hmm_estimate_CN_multi after re-standardization
...
```{r}
dosages <- call_hmm_dosages(hmm_CN = final_res, n.cores = 2)
head(dosages)
```
...
`export_mappoly()` converts the per-window copy-number calls into the input format expected by the
`filter_aneuploidy()` function in the [MAPpoly](https://github.com/mmollina/MAPpoly) package.
It returns a data frame with one row per sample, indicating the estimated CN for each chromosome (column), which
`filter_aneuploidy()` uses to remove chromosomes from aneuploid regions before linkage map construction.
