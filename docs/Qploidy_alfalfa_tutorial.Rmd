...
![](batch_OneMarker2.png)

![](batch_OneMarker3.png)
...
We implemented two ways of performing it, one "quick and dirt" based on parameters grid
...
Following this tutorial, you will be able to apply and evaluate the differences between both approaches. 
...
Thanks to our collaboration with `nQuack` (thank you, Dr. Gaynor!), 
we have functions to easily convert data formats between software:

```{r}
nQuack_list <- convert2nQuack(qploidy_standardization, seed = 123L)
```

Note that nQuack randomize which allele is considered the B allele in the BAF. Set seed
using the argument to reproduce same randomization for exact reproducibility.

Here, we will follow the steps in nQuack:

- Run Normal, Beta and Beta Binomial models for samples with known ploidy (samples you defined in the `genos` object)
- Check which model return the correct ploidy with higher difference between best and second best estimation (BICdif)
- Use selected model for all samples and windows in downstream analysis

This first step may take some time to run depending on the number of cores and samples
you run. We suggest to adapt the number of cores and run it in a high-performance computing system. 
If you just want to follow along with this tutorial in your local computer for now, load the results using the code below.

```{r}
samples <- names(nQuack_list)

# Filter by only known ploidy samples contained in genos object
samples_genos <- unique(genos$SampleName)

# Because we are evaluating a mapping population all samples is expected to be 4x
# We will run nQuack models for all them
all(samples %in% samples) 

em_resultsNormal <- em_resultsBeta <- em_resultsBetaBinom <- list()
for(i in 1:length(samples)){
  em_resultsNormal[[i]] <- quackNormal(xm = nQuack_list[[i]],
                                 samplename = samples[i],
                                 cores = 3,
                                 parallel = TRUE)
  
  em_resultsBeta[[i]] <- quackBeta(xm = nQuack_list[[i]],
                               samplename = samples[i],
                               cores = 3,
                               parallel = TRUE)
  
  em_resultsBetaBinom[[i]] <- quackBetaBinom(xm = nQuack_list[[i]],
                                    samplename = samples[i],
                                    cores = 3,
                                    parallel = TRUE)
}
...
If you don't want to wait for the result to move forward in the tutorial, get the results with:
...
It is important to notice that `nQuack` models will always test ploidies 2 to 6.

Now, we use function `quackit` to interpret model output:

```{r}
summary <- quackit(model_out =  em_results_df)
summary
```
